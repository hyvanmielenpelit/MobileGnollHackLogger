...
- **Layout**: A `.gh-dialog-fullscreen` dialog. The provenance facts (capture method, character count, capture time, GnollHack version, source chat) form one compact row with the SHA-256 row and its copy button under it; **Notes** is an open `<details>` disclosure and the assessor **Digest** a closed one, so the board text starts directly under the toolbar. The board text region is the only scroll container.
...
- **Other Tools**: *Copy Text* (the whole board, unchanged), *Download .snapshot.txt*, and *Edit Metadata*.
