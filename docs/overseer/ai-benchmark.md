...
*Format*, *From a Snapshot*, *Examples* and *For an AI*. It makes **no server call**: its guide
text is static, in `suite-yaml-guide.ts`, and its *For an AI* tab holds the **prompt builder**
...
2. **In an agent session** with the repositories on disk — Claude Code, Antigravity or similar —
   paste the prompt generated by the prompt builder on the Manage Suites help dialog (*For an AI*).
