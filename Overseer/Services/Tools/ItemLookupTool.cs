    ...
    public class ItemLookupTool : IToolHandler
    {
        ...
        internal string BuildMissContent(string name)
        {
            try
            {
                ...
                sb.Append(" Try get_item_stats for the numeric stats straight from the source, or wiki_search with a broader query.");
                return sb.ToString();
            }
            catch
            {
                return $"No information found for item: {name}";
            }
        }
