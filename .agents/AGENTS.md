...
**The cost is accepted deliberately.** Reading all five is roughly 1,350 lines of context up front. That is a considered trade against a session that spent far more than that on subagents rediscovering a subset of the same material and still got a finding wrong. Do not "optimise" this rule back into a conditional one. The fifth, `server-wiki-handoff`, was added on 2026-09-10 after the run-34 wiki handoff shipped with an unverified target page and a presumed generator; its body binds only when a finding lands on ladder rung 2, where its § 4a can pause a round until the user confirms that the wiki change passed validation against the GnollHack source and the wiki session has finished.
