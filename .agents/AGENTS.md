...
- **Fallback**: if no root resolves, write to this repository's `.plans/YYYY-MM-DD/task_name/` and **say so in chat**, naming the reason and the intended scope. Never fall back silently. This is available only because `.plans/` is gitignored here — confirm with `git check-ignore -q .plans`. Where it is not ignored, the plan stays in the chat and no file is written.
