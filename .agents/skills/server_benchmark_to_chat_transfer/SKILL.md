...
repository, and verbatim in this file's Git history before the runs 57 and 59 round.
...
### Run 55 — 2026-09-18: Gemini 3.7 Flash (`medium`), snapshot suite 8
...
- **Grading regime**: harness 32, scoring 11, Standard profile; Claude 5 Opus @ `medium` assessor;
  GPT-5.6 Luna @ `xhigh` as blind second reader and as claim verifier.
- **Instrument SHAs**: prompt `4cc4361b…`, ToolGuides `1fb90bc0…`, KB `576ca574…`, wiki
  `23aba77f…`, source `5bcbab26…`. Board 16,708 chars, `a21dc90405b6`, format 3.
- **Quality**: II 64 ± 7 (evidence-informed 68; hand counterfactual ≈ 71–72). Accuracy 64.8,
  Completeness 55.3. 1 critical error (Q3, genuine: a player-named wand asserted as teleportation).
  **Graded on unrepaired rubrics: the run-54 repair file had not been imported.**
- **Speed**: median model time 8,169 ms, P90 16,095; TTFT P50 2,148 ms.
- **Cost**: 55 tool calls, all succeeded; 3.8 model calls per question at 24,569 input tokens;
  cache read 77.4 %. $2.53: candidate $0.40, assessor $1.50.
- **Verification Outcome (run-54 round, R1)**: A1 verified (found the four stale quotes), A3/A4/A6/A7
  met, H3 not fired; A3 missed in spirit on Q17 (a charged sentence split into fragments); **A5
  failed on Q12** — refuted twice from unreachable `priest_talk`. **T1 reproduced** (Q16).
- **Verifier caution, instances 16–18**: Q12 ×2 (unreachable code, despite instruction 3d); Q5 refuted
  *"lizard corpses for confusion"* while citing the lines that implement it (`src/eat.c:1670-1674`).
- **Transfer Action**: harness 32 → **33** and **scoring method 11 → 12**
  (`docs/overseer/ai-benchmark.md` § *Harness Version 33 & Scoring Method Version 12 Updates*): the
  assessor's own sentences verified under their own role, accused quotes widened to their sentence
  with the charge attached, a citation-liveness note on verdicts that cite a function with no live
  call site, own-knowledge Accuracy deductions reported as `Suspected false:` claims instead of
  lowering the level, re-run provenance, the board's snapshot format recorded, each missing board
  quote named, and a pre-launch board-quote warning. Rubric repair S1–S16 imported by the user
  (verified 2026-09-19: all 14 questions, no missing quote). W1 wiki handoff issued (casting
  attribute of the Clerical and Transmutation schools), not yet run. **The isolated R1b/R2 pair was
  not run, by the user's decision**: the run-54 plan's tool-guide change (T1, rung 3) was not applied
  and T1 stays motivated-only; T2 and T4 motivated-only; T5 went to the GnollHack snapshot plan (G7, landed in GnollHack at `5cdd2c284`).
  Details: `benchmark_run_55_analysis/` (2026-09-19).
- **Pre-declared criteria for the next suite-8 run** (a smoke run under harness 33 and method 12, not
  a comparison with any earlier run):
  - **Method boundary** — no historical run relabelled or mixed; a grading operation on run 55 is
    refused.
  - The manifest names no missing board quote and prints the board's format.
  - No `assessorStatement` item is listed under *Refuted Claims*, and no answer carries
    `RefutedClaim` solely on one; hand-read every assessor statement's verdict.
  - Every accused submission is a whole sentence or list item; no two submissions of one answer
    overlap.
  - Any verdict citing a function without a live call site carries the citation note and raises no
    flag.
  - A re-executed answer, if any, shows its original status; the delivery line shows both stamps.
  - **Method 12** — at most one Accuracy level below 6 whose evidence rests on own knowledge alone
    (with or without the `Not in rubric:` marker), hand-checked; three or more → a wording round under
    a **new** method version, never a silent edit of method 12. Every `Suspected false:` entry has a
    verifier verdict. **Cost guard**: verifier cost above US$0.35 with an unchanged verifier
    configuration → cap the entries per answer at 3 in the prompt, under a new method version.
  - Still binding: the H3 human check on accepted withdrawals, and its rollback
    (`Benchmark:EvidenceInformedRegrade:Enabled` = `false`).
- **Still outstanding from earlier rounds**: the runs 47–48 criteria and the runs 43–46 calibration
  re-assessment. Do not change `RecommendedModels` or treat runs 43–55 as a basis for model
