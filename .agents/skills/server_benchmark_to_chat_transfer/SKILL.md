...
`hyvanmielenpelit/MobileGnollHackLogger/` in the plans repository, and verbatim in this file's Git
history before the run-55 round.
...
### Run 53 — 2026-09-18: Gemini 3.7 Flash (`medium`), snapshot suite 8
...
- **Grading regime**: harness 30, scoring 10, Standard profile; Claude 5 Opus @ `medium` assessor;
  GPT-5.6 Luna @ `xhigh` as blind second reader and as claim verifier.
- **Instrument SHAs**: prompt `4cc4361b…`, ToolGuides `257f305d…`, KB `576ca574…`, wiki
  `1f3fdd54…`, source `b5b7d32f…`. Board 12,037 chars, SHA-256 `8f8c4778d449`, old format.
- **Quality**: II 60 ± 10 (raw 63; contested-verdict 61, evidence-informed 63, verification-cleared
  60, FORM-cleared 60). Accuracy 62.1, Completeness 54.2, Conciseness 92.7, Readability 83.7. 3
  applied critical errors (Q5, Q7, Q11; Q11 contested); 6 refuted claims on 5 answers. The v2
  analyst's instrument-corrected estimate of ≈ 65–67 is a hand counterfactual, not a measurement.
  Compared with run 52 qualitatively only: harness, tool guides, item revisions and assessed
  difficulties all differ.
- **Speed**: median model time 20,362 ms, P90 50,014 ms, max 82,395 ms; TTFT P50 3,740 ms.
- **Cost**: 89 tool calls, all succeeded (35 `source_code_search`, 26 `wiki_search`, 13
  `source_code_view`, 6 `wiki_view`); 4.9 calls per question, 5.1 rounds per tool-using question, 1.1
  calls per round. 2.35 M input tokens, cache read 79.4 %. $2.94: candidate $0.53, assessor $1.81,
  second reader $0.19, verifier $0.16, synthesis $0.25.
- **Transfer Action**: harness 30 → **31** and **scoring method 10 → 11**
  (`docs/overseer/ai-benchmark.md` § *Harness Version 31 & Scoring Method Version 11 Updates*).
  - **H0**: grading operations are refused on a run graded under another method. Rescore stays and
    never stamps 11 onto levels it did not grade.
  - **H1**: accused sentences are verified, with server-owned roles, candidate tool calls as untrusted
    leads, and a new verdict clearing the old verification.
  - **H2**: ACCURACY levels 4–6 re-anchored on what the answer states, plus the precision detector.
  - **H3**: the evidence-informed re-grade is validated against a target catalog, with one eligibility
    predicate. **H3's rollback trigger fired on run 52's round criterion (Q11, Q12) and was answered
    by a decision to repair and leave enabled, not erased**; it stands again for the next run, read
    against withdrawals that survive validation.
  - **H3b** (critical-error quote), **H4** (denial-scoped detectors), **H5** (math judged as typeset),
    **H6** (row-aware tool diagnostics).
  - Rung 3: **T1** `source_code_search` unfiltered probe on a filtered miss; **T2** coverage-first
    `wiki_search` ranking; **T3** parenthesised flag unions; **T4** `get_artifact_stats.md`
    base-item paragraph. These move `ToolGuidesSha256`.
  - **H7** (second-reader latency) undecided; **H8** (request-prefix measurement) recorded with its
    real completion status in the round's walkthrough.
  - Six rubric repairs (Q2, Q6, Q13, Q15, Q16, Q18) accepted as an import YAML and **need a confirmed
    import**. G1 is already in snapshot format 3; the remaining GnollHack snapshot changes target
    format 4.
  - No chat system prompt change, no wiki edit, no corpus-health conclusion.
- **Comparability reset: `ScoringMethodVersion` 10 → 11.** Accuracy levels 4–6 now anchor on what the
  answer states rather than on source-level depth, so a v10 run is not comparable with a v11 run on
  Accuracy, and therefore on the quality score and every index. Scores rise across the boundary, and
  the rise is not evidence of anything. The comparison view refuses to rank the two, because both
  `HarnessVersion` and `ScoringMethodVersion` moved.
- **Pre-declared criteria for this round's first run** (a smoke run under harness 31 and method 11,
  not a comparison with run 53; fix the grader roster first and record it):
  - **Method boundary** — no historical run relabelled or mixed.
  - **H1** — read every new accusation. Verifier cost above **$0.24** on a suite-8 run with an
    unchanged verifier configuration → set `Benchmark:ClaimVerification:AccusedQuotesEnabled` to
    `false` pending diagnosis.
  - **H2** — at most 1 Accuracy level below 6 with no named statement, hand-checked; 3 or more → a
    follow-up wording round under a **new** method version, never a silent edit of method 11.
  - **H3** — two answers in one run with a semantically unsupported **accepted** withdrawal →
    `Benchmark:EvidenceInformedRegrade:Enabled` = `false`.
  - **T1** — no question repeats an unchanged filtered miss (a target, not a guarantee).
  - **T2 and T4** need an isolated-variable pair with held-out questions and authorised spend;
    without one their behavioural acceptance is **pending**, not passed.
