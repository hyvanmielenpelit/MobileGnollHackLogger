...
### Runs 60 and 61 — 2026-09-19: Gemini 3.7 Flash (`medium`) and GPT-5.6 Luna (`high`), snapshot suite 8
- **Candidates**: Gemini 3.7 Flash @ `medium` (run 60); GPT-5.6 Luna @ `high` (run 61); parallel
  mode 2.
...
- **Grading regime**: harness 35, scoring 12, Standard profile; Claude 5 Opus @ `medium`; Gemini 3.8
  Flash @ `medium` as second reader and claim verifier — **the user's deliberate roster from this
  round on**, chosen because it is a different model from both candidates. The GPT-5.6 Luna roster
  that the run-56 and runs-57/59 runbooks asked for is retired; **do not ask for it again**, and do
  not read this roster as a missed runbook step.
- **Instrument SHAs**: prompt `3ec0287f…`, ToolGuides `f0c53b98…`, KB `576ca574…`, wiki
  `fca2ddfe…`, source `04e69994…`. Board 16,708 chars, `a21dc90405b6`, format 3.
- **Quality**: II 74 ± 9 and 71 ± 8 (hand counterfactual ≈ 75 and ≈ 72). Critical errors 2 (Q3, Q11,
  both genuine) and 1 (Q11, genuine).
- **Speed**: median model time 9,085 and 15,510 ms; TTFT P50 1,950 and 2,125 ms.
- **Cost**: US$3.97 and US$2.54; candidate US$0.45 and US$0.08; verifier US$1.93 and US$1.21 (49 %
  and 48 %, about US$0.04 per checked item). 48 and 67 tool calls, all succeeded; 1.1 and 1.7 calls
  per round; cache read 76 % and 92 %.
- **Verification Outcome (runs 57 and 59 round)**: T1 and T2 met; **T3 missed, rollback not tripped,
  sentence kept** (recorded as *policy text does not move this model*); H1 two parts met and the
  `OutOfRubricAccuracyDeduction` part missed; H2 retired with the roster it was written for; S1–S5
  met; method-12 trigger not fired (2 and 1).
- **Verifier caution, instances 29–35**: 61 Q11 ×3 (lizard corpse resistance read from the
  post-effect; magic cancellation against the touch; a header line of another function cited), 61 Q16
  (failure path cited against a success claim), 60 Q7 ×3 (two refutations and one **wrong support** on
  a bare file name). Six of seven side with NetHack.
- **Transfer Action**: harness 35 → **36** (`docs/overseer/ai-benchmark.md` § *Harness Version 36
  Updates*): two citation notes in `BenchmarkCitationLivenessCheck` — a source file cited without a
  line, and a single cited line that is only a function's definition line — each read as
  Indeterminate; claim-verification instruction 5 says what a citation is and instruction 3i asks for
  every place an effect could be applied before absence is concluded; the report's *Suspected false*
  outcomes read *(the verifier sided with the assessor / with the answer)*. Rung 3 **T1**: a
  categorised `wiki_search` whose best unfiltered match lies outside the category ends with a
  *[Without category, the best match for this query is …]* line, described in `wiki_search.md`, which
  moves `ToolGuidesSha256`; `CandidateSystemPromptSha256` stays `3ec0287f…`. Rung 2 **W1** wiki handoff
  (`Spell Casting.md` § *Learning spells*), not yet run; rubric repair S1–S4 (Q16, Q11, Q7, Q18; custom
  suite, no seed file); GnollHack plan N1–N2 (stairs destinations, castability of an unlearned
  spellbook), deferred until the snapshot work finishes. No change to `ChatService.BuildSystemPrompt`
  or `_policy.md`; no `RecommendedModels` change; scoring method 13 not written (trigger not fired).
- **Pre-declared criteria for the next runs**: `benchmark_runs_60_61_analysis_v2.md` § 5 — including
  the verifier cost criterion, re-declared **per checked item** (at or below US$0.045 with Gemini 3.8
  Flash @ `medium` in both grader roles).
