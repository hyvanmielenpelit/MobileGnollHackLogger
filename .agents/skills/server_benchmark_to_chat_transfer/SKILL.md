...
verbatim in this file's Git history before the run-56 round.
...
### Run 54 — 2026-09-18: Gemini 3.7 Flash (`medium`), snapshot suite 8
...
- **Grading regime**: harness 31, scoring 11, Standard profile; Claude 5 Opus @ `medium` assessor;
...
- **Instrument SHAs**: prompt `4cc4361b…`, ToolGuides `1fb90bc0…`, KB `576ca574…`, wiki
  `1f3fdd54…`, source `5bcbab26…`. Board 16,708 chars, SHA-256 `a21dc90405b6`, format 3 — **not
  the board runs 52–53 read** (12,037 chars, `8f8c4778d449`); no comparability key records that.
- **Quality**: II 71 ± 4 (evidence-informed 72, verification-cleared 71; hand counterfactual
  ≈ 75–76). Accuracy 71.2, Completeness 57.7, Conciseness 95.7, Readability 87.0. 0 critical
  errors; 8 refuted claims on 6 answers. First method-11 run: no comparable predecessor.
- **Speed**: median model time 14,760 ms, P90 35,339 ms; TTFT P50 4,596 ms, P90 16,898 ms.
- **Cost**: 57 tool calls, all succeeded (21 `wiki_search`, 14 `source_code_search`, 10
  `source_code_view`); 3.5 rounds per tool-using question, 1.1 calls per round. 1.74 M input
  tokens, cache read 79.8 %. $3.07: candidate $0.39, assessor $2.05.
- **Verifier caution, instances 14–15**: Q12 refuted from **unreachable code** (`priest_talk`,
  whose only callers are commented out at `src/sounds.c:1123`, `:7609`); Q11 **Supported** a
  false claim from a wiki page alone against `src/timeout.c:303-349`.
- **Transfer Action**: see `benchmark_run_54_analysis/` (2026-09-18).
