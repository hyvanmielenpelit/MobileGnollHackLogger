...
- **Two advisory sensitivity indices sit beside the Intelligence Index, and neither is a score.** *Contested-Verdict Sensitivity* resolves every critical-error split at the second reader's figure. From harness 25, *Verification-cleared Accuracy Sensitivity* recomputes the index with Accuracy one level higher on every answer whose Accuracy was docked citing only claims the rubric did not cover, where the verifier later supported every such claim it checked — the instrument's share of the Accuracy shortfall, as `OUT-OF-SCOPE:` is of Completeness. Both are omitted when their population is empty, and neither moves a stored number. Read them as the width of the grading's own uncertainty, never as a corrected result. From harness 30 a third, *Evidence-informed Sensitivity*, recomputes the index with the primary assessor's evidence-informed re-grade (the same assessor re-reading a contested answer with the claim verifier's findings in hand) on the answers that received one. Like the other two it is advisory and never a score: it is stored apart from every verdict, read by no scoring path, and omitted when no answer was re-graded. Before relying on it, read each re-graded answer's `withdrawn` list against the verifier verdicts it cites, because the verifier it leans on has its own recorded failure modes (§ 11). From harness 31 the figure counts **validated** re-grades only — a withdrawal must name a server-listed target on the findings listed for it — and prints how many re-grades it excluded; on an earlier run it is labelled *legacy, unvalidated*.
- **What Accuracy 6 means depends on the scoring method.** From scoring method 11 (harness 31) Accuracy grades only what the answer states: level 6 is *no false or imprecise statement in any claim the assessor can adjudicate*, and depth, length and source-level detail are Completeness matters. Under method 10 and earlier, levels 5 and 6 rewarded source-level depth that the concise production prompt tells the candidate not to produce. A method-11 Accuracy, quality score or index is therefore not comparable with an earlier one, and a rise across that boundary says nothing about the candidate.
- **From scoring method 12 (harness 33) Accuracy is graded against the rubric and the game board only.** A statement the assessor believes false from its own knowledge, which neither settles, no longer lowers the level: the assessor reports it as an `unverifiedClaims` entry prefixed `Suspected false: `, the claim verifier checks the answer's sentence, and a refutation raises `RefutedClaim` and routes the answer to the second reader as before. The report counts those entries apart (*"Suspected false by the assessor: N — refuted a (the verifier sided with the assessor), …"*; before harness 36 the report said *"(the assessor was right)"*, which states a verdict of a fallible verifier as fact). Under method 11 the same deduction lowered the scored level, and it was wrong on 6 of 8 answers of run 55 and 7 of 18 of run 53 — NetHack priors charged against GnollHack facts. A true out-of-rubric error therefore no longer costs Accuracy until a verifier refutes it, which is the accepted trade. An `OutOfRubricAccuracyDeduction` flag on a method-12 run means the assessor docked on own knowledge **against its instructions**. Method-12 scores generally rise across the boundary, which says nothing about a candidate; no method-12 figure is comparable with a method-11 one, on any suite.
- **The board is part of the exam.** From harness 32 the game snapshot a run read (`GameSnapshotSha256Used`) is a **Fundamental** comparability key, `GameSnapshot`: two runs of one snapshot suite on different boards are not pooled or ranked. Before 32 no key recorded it, which is how runs 53 and 54 read different boards unnoticed. Adding the key changed every run's `ComparabilityKeyHash`, so a group analysis stored before it should be re-analysed. From harness 32 the board also sits in its own system message ahead of the question for every grading role, so a 31-stamped and a 32-stamped snapshot-suite run are not grade-comparable either.
- **Interpretation**:
...
- **Interpretation**:
  ...
  - Gemini candidate output-token and cost figures before harness 34 exclude thinking tokens (`thoughtsTokenCount` was not added to `OutputTokens`), so they under-state the candidate's cost and are not comparable on cost with later runs.
...
| 50–51 | 2026-09-17 | Gemini 3.7 Flash / GPT-5.6 Luna, snapshot suites | not used for selection | **Exposed the harness-29 wire defect** (H1): `BuildCandidateSeedHistory` put the board ahead of the system prompt, so a Google or Anthropic candidate on a snapshot suite received no board and an OpenAI candidate received no system prompt at all (runs 36, 37, 38, 42, 46, 47, 51 — see the correction below). Delivered: the system prompt first, `BenchmarkCandidateRequestProbe` (per run fail-closed, per question fail-the-answer), the board to the claim verifier (instruction 3c, `board:` citations), and T1–T3 tool-contract miss payloads — `HarnessVersion` 28 → 29, no fingerprinted file moved. **Explicitly not a reproduction pair**, and neither is usable for model selection. Its harness criteria were read off run 52: H1 and H2 met, T1 vacuous, T2 met, T3 not exercised. |
| 52 | 2026-09-18 | Gemini 3.7 Flash (`medium`), snapshot suite 8 | 54 ± 9 | First run of snapshot suite 8 (board 12,037 chars, `8f8c4778d449`, old format); harness 29, method 10, assessor Claude 5 Opus @ `medium`, GPT-5.6 Luna @ `xhigh` as second reader and verifier. 4 critical errors (Q3, Q5 genuine; Q11, Q17 split); 3 refutations, 2 wrong (verifier caution 12–13). The second opinion took 21 m 17 s of 23 m 18 s. Transfer: harness 29 → **30** — every grading path loads the board and `BenchmarkBoardGuard` refuses one that did not, delivery recorded per role, the advisory evidence-informed re-grade, a list-item critical-error quote verified under its heading, re-runs without recorded prompt options refused; rung 3 `wiki_search` lead block (moved `ToolGuidesSha256`); T3 wiki handoff; S1–S3 rubric YAML. T4 (one tool call per round) and T6 (unlooked-up mechanics wrong) motivated-only; T5 to the GnollHack snapshot plan. Its round criteria were read off run 53. |
| 53 | 2026-09-18 | Gemini 3.7 Flash (`medium`), snapshot suite 8 | 60 ± 10 | Harness 30, method 10, same roster as run 52 (Luna @ `xhigh`); board 12,037 chars, `8f8c4778d449` — **not the board runs 54–56 read**, which no key recorded until harness 32. 3 applied critical errors (Q5, Q7, Q11; Q11 contested); the assessor called true, tool-grounded GnollHack facts "invented" on 7 of 18 answers. Transfer: harness 30 → **31** — H0 grading operations refused across a method boundary, H1 accused sentences verified with server-owned roles, H2 Accuracy 4–6 anchored on what the answer states plus the precision detector, H3 the validated evidence-informed re-grade (its rollback trigger fired and was answered by repair, not removal), H3b–H6; rung 3 T1–T4 (moved `ToolGuidesSha256`); six rubric repairs. **Comparability reset: `ScoringMethodVersion` 10 → 11**, with `HarnessVersion` moving too, so the comparison view does not rank across it; and the board changed between runs 53 and 54, so no run-53 figure is comparable with runs 54–56 on any suite-8 measure. $2.94; median model time 20,362 ms. |
| 54 | 2026-09-18 | Gemini 3.7 Flash (`medium`), snapshot suite 8 | 71 ± 4 | Harness 31, method 11, Claude 5 Opus @ `medium` assessor, GPT-5.6 Luna @ `xhigh` second reader and verifier; first run on the 16,708-char board `a21dc90405b6`, format 3, which no key recorded until harness 32 — so it is not comparable with runs 52–53 on any suite-8 measure. First method-11 run, no comparable predecessor; hand counterfactual ≈ 75–76. 0 critical errors; 8 refuted claims on 6 answers. Verifier caution 14–15: Q12 refuted from unreachable `priest_talk` (`src/sounds.c:1123`, `:7609`), Q11 Supported from a wiki page against `src/timeout.c:303-349` — the origin of instructions 3d and 3e. 57 tool calls, all succeeded; median model time 14,760 ms; $3.07. Transfer action in `benchmark_run_54_analysis/` (2026-09-18). |
| 55 | 2026-09-18 | Gemini 3.7 Flash (`medium`), snapshot suite 8 | 64 ± 7 | Harness 32, method 11, Claude 5 Opus @ `medium` assessor, GPT-5.6 Luna @ `xhigh` second reader and verifier; board 16,708 chars, `a21dc90405b6`, format 3. **Graded on unrepaired rubrics: the run-54 repair file had not been imported** — the origin of the check-the-prior-import step. Hand counterfactual ≈ 71–72. 1 critical error (Q3, genuine: a player-named wand asserted as teleportation). Verifier caution 16–18 (Q12 ×2 from unreachable `priest_talk` despite instruction 3d; Q5 refuted while citing the implementing lines). Transfer: harness 32 → **33** and **scoring method 11 → 12** — own-knowledge Accuracy deductions reported as `Suspected false:` claims instead of lowering the level, the assessor's own sentences verified under their own role, accused quotes widened to their sentence, the deterministic citation-liveness note, re-run provenance, the board's format and each missing board quote recorded. Rubric repair S1–S16 imported; the isolated R1b/R2 pair was not run, by the user's decision. Still binding from its criteria: the human check on every accepted evidence-informed withdrawal, and its rollback (`Benchmark:EvidenceInformedRegrade:Enabled` = `false`). 55 tool calls, all succeeded; median model time 8,169 ms; $2.53. |
| 56 | 2026-09-19 | Gemini 3.7 Flash (`medium`), snapshot suite 8 | 70 ± 9 | Harness 33, method 12 — the first method-12 run, no comparable predecessor; Claude 5 Opus @ `medium` assessor, GPT-5.6 Luna @ `high` second reader and verifier; board 16,708 chars, `a21dc90405b6`, format 3. Hand counterfactual ≈ 72. 2 critical errors (Q3 contested, Q7 confirmed), both on answers with no tool call. Verifier caution 19–22, the origin of instructions 3f–3h. Transfer: harness 33 → **34** — Gemini thinking tokens counted as output (its own candidate cost of $0.34 is under-counted), the definition pointer in `source_code_search` and `source_code_view` (T1, rung 3), the liveness check's body and value-reference rules, a docked `Suspected false:` sentence flagged; rubric repair S1 (Q14); wiki handoff W1–W2; GnollHack plan G1–G4. **Method 12's trigger fired** (four own-knowledge deductions) and the wording round was deferred by the user's decision of 2026-09-19. 53 tool calls, all succeeded; median model time 8,322 ms; $1.87. |
| 57, 59 | 2026-09-19 | Gemini 3.7 Flash (`medium`) / GPT-5.6 Luna (`high`), snapshot suite 8 | 67 ± 11 / 67 ± 10 | Harness 34, method 12, Claude 5 Opus @ `medium` assessor; **the first runs with Gemini 3.8 Flash @ `medium` as second reader and claim verifier**, which became the user's deliberate roster from the next round on. Board 16,708 chars, `a21dc90405b6`, format 3; prompt `4cc4361b…`, ToolGuides `1fb90bc0…`. Hand counterfactual ≈ 71–72 and ≈ 69–70. Critical errors 3 (Q7 spurious, Q11 overreach, Q3 genuine) and 2 (Q11, Q16 genuine). Verifier caution 23–28 (a function header cited as the body, one fact refuted and supported in one run, a sequence read as present tense, a misread cited table, a substituted subject, a cited file that does not exist). Transfer: harness 34 → **35** — detector vocabulary, the not-in-the-index liveness note, duplicate-claim removal; rung 3 T1 (`wiki_search.md` categories), T2 (compiled-out note in the source readers), T3 (one `_policy.md` sentence, later recorded as not moving this model); rubric repair S1–S5; wiki handoff W1 under a validation template that fails only on a factual, cited wording problem. Verifier $2.06 and $1.66 (53 %); $3.92 and $3.14; median model time 10,578 and 16,397 ms. |
| 60, 61 | 2026-09-19 | Gemini 3.7 Flash (`medium`) / GPT-5.6 Luna (`high`), snapshot suite 8 | 74 ± 9 / 71 ± 8 | Harness 35, method 12, Claude 5 Opus @ `medium` assessor, Gemini 3.8 Flash @ `medium` second reader and verifier — **the user's deliberate roster from this round on; the GPT-5.6 Luna roster is retired, do not ask for it again**. Board 16,708 chars, `a21dc90405b6`, format 3; prompt `3ec0287f…`, ToolGuides `f0c53b98…`. Hand counterfactual ≈ 75 and ≈ 72. Critical errors 2 (Q3, Q11, genuine) and 1 (Q11, genuine). Verifier caution 29–35, six of seven siding with NetHack. Transfer: harness 35 → **36** — the file-without-a-line and definition-line citation notes, instructions 5 and 3i, *Suspected false* outcome wording; rung 3 T1 *Without category* line (moved `ToolGuidesSha256`; rollback later tripped); wiki handoff W1; rubric repair S1–S4; GnollHack plan N1–N2. Runs 57/59 round verified except T3 (kept, *policy text does not move this model*). Verifier $1.93 and $1.21 (about $0.04 per checked item); $3.97 and $2.54; median model time 9,085 and 15,510 ms. |
...
the plans repository, and verbatim in this file's Git history before the run-52 round. The full entry
for run 52 — its pre-declared T1, harness and H3 criteria, and the verification of the harness-29
round — is likewise in `benchmark_run_52_analysis/` (2026-09-18) under
`hyvanmielenpelit/MobileGnollHackLogger/` in the plans repository, and verbatim in this file's Git
history before the run-55 round. The full entry for run 53 — its tool-call breakdown, the harness-31
transfer action item by item and its pre-declared criteria — is likewise in the run-53 round's task
directory (2026-09-18) under `hyvanmielenpelit/MobileGnollHackLogger/` in the plans repository, and
verbatim in this file's Git history before the run-56 round. The full entry for run 54 — its tool-call
breakdown by tool, its dimension scores and its speed percentiles — is likewise in
`benchmark_run_54_analysis/` (2026-09-18) under `hyvanmielenpelit/MobileGnollHackLogger/` in the plans
repository, and verbatim in this file's Git history before the runs 57 and 59 round. The full entry
for run 55 — the run-54 round's verification item by item, the harness-33 and method-12 transfer
action and its pre-declared criteria with the method-12 cost guard — is likewise in
`benchmark_run_55_analysis/` (2026-09-19) under `hyvanmielenpelit/MobileGnollHackLogger/` in the plans
repository, and verbatim in this file's Git history before the runs 60 and 61 round. The full entry
for run 56 — its prompt options, dimension scores, the verifier-caution instances 19–22 with their
source lines and the deferred method-12 wording round — is likewise in `benchmark_run_56_analysis/`
(2026-09-19) under `hyvanmielenpelit/MobileGnollHackLogger/` in the plans repository, and verbatim in
this file's Git history before the runs 62 and 63 round. The full entry for runs 57 and 59 — their
instrument SHAs, the run-56 round's verification item by item, the verifier-caution instances 23–28
and the harness-35 transfer action — is likewise in `benchmark_runs_57_59_analysis/` (2026-09-19) under
`hyvanmielenpelit/MobileGnollHackLogger/` in the plans repository, and verbatim in this file's Git
history before the runs 64 and 65 round.

Seven standing cautions from these entries, kept because they still bind:

- **The scoring-profile comparability key was widened, then narrowed (H9, resolved 2026-09-07).** It now hashes `BenchmarkScoringProfileService.CanonicalSignature` — the profile's scoring semantics only — recomputed from each run's stored snapshot, so nothing that matched before stopped matching. Every stored **group analysis** should still be re-analysed so its recorded `ComparabilityKeyHash` reflects the narrowed definition. **Comparability key definition version 2 (2026-09-24)** moved the keys again: candidate and grader settings are read from recorded model snapshots, `CandidateEndpoint` joined the candidate keys, and all three grader signatures gained the effective output cap and the endpoint and lost `parallelMode`. Every group analysed before it shows *Comparability definition changed* until re-analysed, and runs recorded before it read as a different grader instrument from later runs, because their unrecorded grader settings render `(not recorded)`.
- **Runs 11–14 and runs 16–21 sit on opposite sides of the v7 → v8 reset**, and everything before run 22 sits on the far side of v8 → v9 (Readability) and v9 → v10 (unanswered questions). Every run up to and including run 53 sits on the far side of v10 → v11 (Accuracy anchors), and every run up to and including run 55 on the far side of v11 → v12 (own-knowledge Accuracy deductions no longer lower the level). Five resets now separate run 11 from the present.
- **A refutation is advisory evidence, and a human reads the cited line before any change rests on it.** On run 24 Q1 the claim verifier produced **two** refutations and **both were wrong**, each with a confident citation, and both are recorded permanently in that report and flagged as `BenchmarkAnswerFlags.RefutedClaim`.
  ...
  - **Now forty-three recorded instances** (fourteenth to forty-third at the end of this bullet). Fourth (run 30, Q11 trident range, `src/apply.c:5233`), fifth (run 33, Q1 wish-repeat odds), sixth and seventh (run 35, Q16 group-size `#if 0` citation and Q7's "d20-style save" wrongly marked Supported) were followed by two more on run 36, both at claim-verifier `low`: eighth, Q7's fear-skill save table refuted against a wiki table that omits the spell's extra per-skill-level term (`src/zap.c:949`); ninth, Q14's −4 magic-cancellation penalty refuted against Master Kaen's `mcadj` data field instead of the code that applies it, `src/mcastu.c:793`. From harness 20 the claim-verification prompt's instruction 3a is the response: a claim about how a spell, attack or effect is computed is checked in the code that implements it, not only a data table or a wiki page that may simply omit the term. **Tenth (run 42, Q15, verifier Gemini 3.7 Flash @ `high`): the claim "after the glyph callback, it separately sends object records … engraving data" was refuted against `include/winprocs.h:58` on the basis that *layer* information is not sent separately — answering a claim the candidate did not make.** `lib_print_glyph` in `win/win32/xpl/libshare/libproc.c` calls `callback_print_glyph` at `:494`, then `callback_send_object_data` at `:580` and `:599` and `callback_send_engraving_data` at `:613`, so the claim is true as written. The failure mode here is not a wrong citation but a **substituted subject**: read what the verifier says it refuted before reading its citation, and treat a refutation whose basis names a different thing than the claim as unverified. **Eleventh (run 48, Q13, verifier Claude 5 Sonnet @ `high`): the claim that fire resistance reduces fire damage by 50 % was refuted against the code that *grants* the intrinsic rather than the code that applies it** — `src/cmd.c:4579` prints the enlightenment string for the half-damage tier, and the resistance rolls in `src/zap.c` are where a magnitude is settled. From harness 28 the claim-verification prompt's instruction 3b is the response: a claim about the magnitude or tier of a resistance or property is checked where the property is applied, because how an intrinsic is acquired says nothing about how much it protects. **Twelfth and thirteenth (run 52, verifier GPT-5.6 Luna @ `xhigh`), both the *data or description instead of the applying code* failure that instructions 3a and 3b already target.** Q6: *"blessing supplies only +1 max HP"* is wrong, because the healing roll is `d(12 + 6*bcsign, 6)`, 18d6 blessed (`src/potion.c:966-969`). Q3: *"the camera only frightens"* is wrong, because it blinds adjacent monsters and scares within distance (`src/uhitm.c:4951-4977`). Neither was a contest, so neither entered the harness-30 evidence-informed re-grade. That stage's population is refutations of the *assessor's* basis and supported critical-error quotes, never a refutation of the candidate. Its human-read check on each `withdrawn` list is still the guard against a wrong verdict flowing the other way. **Fourteenth and fifteenth (run 54, verifier GPT-5.6 Luna @ `xhigh`), one in each direction.** Q12 was refuted from **unreachable code**: `priest_talk`, whose only callers are commented out at `src/sounds.c:1123` and `:7609`, so it decides nothing. Q11 was **Supported** from a wiki page alone, against the timer code that applies the number (`src/timeout.c:303-349`). From harness 32 the claim-verification prompt's instructions 3d (confirm a cited function has a live call site) and 3e (a number is settled in the code that applies it; a wiki page alone does not settle it while the source is searchable) are the response. **Sixteenth to eighteenth (run 55, same verifier): 3d did not hold.** Q12 was refuted twice more from `priest_talk`, and Q5 refuted *"lizard corpses for confusion"* while citing the very lines that implement it (`src/eat.c:1670-1674`). Prose was not enough, so from harness 33 a deterministic check stands behind 3d: `BenchmarkCitationLivenessCheck` resolves the function enclosing a cited `src/*.c` line and, when it has no live call site in the indexed GnollHack source, adds the note *"cited function <name> has no live call site"* and counts the verdict as Indeterminate for every flag and count. The stored verdict text is kept and both are printed. The check cannot see a function reached only through a function pointer or a macro, which is why it demotes to Indeterminate and never to the opposite verdict. A wrong verdict that cites live code, like Q5's, is not caught by it. **Nineteenth to twenty-second (run 56, verifier GPT-5.6 Luna @ `high`), three new shapes.** Q3's camera blinding was refuted again from `use_camera` without reading `flash_hits_mon`, the function it hands the effect to (`src/uhitm.c:4936-4977`). Q13 was refuted twice because a zero-based skill index was read as a different formula (`src/spell.c:5267-5281`). Q14's accused sentence was refuted from the general `can_advance` code, although the Monk's Martial arts has no experience-level route (`src/role.c:501`). From harness 34 the claim-verification prompt's instructions 3f (facts, not advice: a recommendation is refuted only through a false mechanic it states), 3g (before refuting a formula, a table or a number, check for the same quantity in another notation and recompute the claim's own worked example) and 3h (read the function the cited one hands the effect to before concluding that an effect is absent) are the response. From the same harness the liveness check needs a definition followed by a body — a macro row in a data table is no function — and counts a value reference such as `set_occupation(learn, …)` as live, so a function reached through a function pointer is no longer noted. **Twenty-third to twenty-eighth (runs 57 and 59, verifier Gemini 3.8 Flash @ `medium`)**: a function header cited as if it settled the body (57 Q7), one fact refuted and supported in the same run (57 Q11), a sequence read as a present-tense claim (57 Q14), the verifier's own cited table misread (59 Q18), a substituted subject (59 Q8), and a cited file that does not exist in the indexed source (57 Q1). From harness 35 a verdict citing a `src/` file that is not in the indexed GnollHack source carries the note *"cited file <path> is not in the indexed source"* and counts as Indeterminate, as the liveness note does. **Twenty-ninth to thirty-fifth (runs 60 and 61, same verifier), six of seven siding with a NetHack assumption against a true GnollHack fact.** 61 Q11: a lizard corpse's 13 turns of resistance refuted from the post-effect (`src/eat.c:1641`; the grant is at `:1141-1151`), magic cancellation against a cockatrice's touch refuted from one function of the attack path (`src/mhitu.c:1340`; true at `:1869-1870`, `:2429-2437`), and an accused *"a failed read does not paralyze"* refuted citing `src/spell.c:5186`, the definition line of `percent_success`, which is about casting. 61 Q16: a success claim refuted from the failure path (`src/spell.c:484`; the success is at `:640-646`). 60 Q7: two refutations and one **wrong Supported** — *"can fail or paralyze you"*, whose paralysis exists only inside `#if 0` — each citing a bare `src/shk.c` or `src/spell.c` with no line; the wrong support made a validated evidence-informed re-grade withdraw a right charge. From harness 36 a verdict whose only source citation is a file without a line carries *"cited file <path> without a line"*, and one whose only cited line is a function's definition line carries *"cited line <path>:<line> is only the definition line of <function>"*; both count as Indeterminate, a range is never noted as a definition line, and a wiki or board citation beside a bare file keeps the verdict. Instruction 5 now says that neither is a citation, and instruction 3i (absence needs more than one place: rule out the pre-effect, the post-effect, the callers and the shared checks before concluding an effect is absent) is the response to the lizard and magic-cancellation shapes. A wrong verdict that cites a line inside the wrong live function is still not caught. **Thirty-sixth (run 63, same verifier): a dispatch block read as the settling code.** Q6's *"healing potions restore 12d6 HP; the blessed one also cures sickness and can increase maximum HP by 1"* was refuted citing `src/potion.c:1784-1793`, the `case POT_HEALING:` block, which only passes values computed at the top of the same function — the dice from `src/objects.c:3313-3317` and `src/potion.c:966-971`, the blessed cure from `:1001-1003`, the extra maximum HP at `:1789`. From harness 37 instruction 3j (values passed are settled where they are assigned: read where the variables a cited call passes are computed, and the object's data entry, before refuting a number, a die roll or a cure) is the response. **Thirty-seventh to thirty-ninth (runs 64 and 65, same verifier): a Supported verdict covering a true clause beside the charged one.** 64 Q11's critical-error quote was Supported on its read-ability clause (`src/spell.c:889-894`) while the charged part, "keep a casting ready", was one the same verifier refuted on another item. 65 Q1's *"dwarves are among the creatures your lawful alignment is expected to respect"* was Supported from `src/mon.c:4935-4953`, the general penalty for angering any peaceful, which has no dwarf rule. 65 Q8's *"you currently lack a reliable … escape option"* was Supported from the board's *Controlled teleport* line, although the hero carries a wand of digging. It differs from the tenth instance's substituted subject: there the verifier answered a claim nobody made, here it answers part of the claim made and ignores the part that was charged. Two of the four validated evidence-informed withdrawals (65 Q1, 65 Q8) withdrew true charges because of it. From harness 38 an accused item prints its *Charged part* (the words the assessor quoted), the accused and critical-error preambles say a true clause elsewhere does not make a false charged part Supported, and instruction 3k asks the basis to say which statement was checked. **Fortieth to forty-third (runs 66 and 67, same verifier).** 66 Q13's *"these four schools are non-somatic"* was refuted citing `src/objects.c:3549`, a parameter line of the `#define SPELL(…)` header — a macro's definition, not evidence; the somatic flag is uniform per school (Clerical 18/18, Healing 10/10, Divination 19/19, Abjuration 24/24, Movement 18/18). 67 Q6's *"healing casts do not consume a turn"* was refuted from `docast` (`src/spell.c:1538-1548`), which only returns `spelleffects`' result, while Minor healing carries `S1_DOES_NOT_TAKE_A_TURN` (`src/objects.c:4523-4526`) and the wiki page the candidate read says "Casting time: 0 rounds". 67 Q7's accused sentence was Supported from `src/read.c:2786`, which settles only the uncharged clause, although harness 38 printed the charged part — the harness-38 prose did not hold. 67 Q15's *"The Grail recharges itself over 1,500 turns"* was Supported from `include/artilist.h:331-336`, reading the constant `ARTINVOKE_RECHARGE_ITSELF` as the claim's meaning. Two of run 67's three validated evidence-informed withdrawals (Q7, Q15) rest on these supports. From harness 39 a verdict whose only source citation is a single line inside a `#define` header carries *"cited line <path>:<line> is only the definition of macro <NAME>"* and counts as Indeterminate; an accused item with a charged part is judged by its own structured `chargedPartVerdict` (omitted means Indeterminate, with the note *"the charged part was not judged separately"*), so a verdict about another clause can no longer count; and instruction 3l asks a verdict that contradicts a GnollHack wiki statement to name that statement and the code that overrides it. The forty-third shape — a constant's name read as the claim's meaning — is caught by none of these and stays a caution.
  - **The same failure has an assessor-side class: a true, tool-grounded fact charged as fabricated.** The assessor grades without tools and never sees what the candidate's tools returned, so on run 53 it called true GnollHack facts "invented" or "fabricated" on seven of 18 answers, and only four were caught by the existing paths. From harness 31 the harness sends a sentence the assessor quoted when it docked Accuracy at 4 or below, or on a contested verdict, to the claim verifier, and a Supported verdict raises the advisory `ContestedAccuracyDeduction`. From harness 38 an Accuracy-5 deduction is verified as well (runs 64 Q7 and 65 Q10 docked true facts to 5 and reached no check). That check is bounded — it finds only accusations the assessor quoted — so an "invented" charge in evidence is still unverified until someone reads the tool-call log and the source. A **Supported** accused sentence is the verdict to distrust here: the recorded verifier errors above are mostly wrong refutations, and a wrong support would clear a genuine error. From harness 33 an accused quote is widened to the whole sentence or list item it came from, with the assessor's charge attached as untrusted context, so a charged sentence is no longer split into fragments one of which comes back Supported (run 55 Q17).
  - **Read a verdict's polarity from its role (harness 33).** Before harness 33, when an answer was contested and the assessor listed no unverified claims, the harness also sent sentences of the assessor's *own* evidence to the verifier as if they were the candidate's claims: a Refuted verdict on one meant the *candidate* was right, yet it counted as a refuted claim, raised `RefutedClaim` and triggered the second opinion. Run 55's Q15 "refuted claim" was of this kind. From harness 33 those items carry the role `assessorStatement`; they count in no ordinary-claim figure, and a Refuted one raises `ContestedAccuracyDeduction` instead. A report of an earlier run lists them as ordinary claims and cannot be corrected by text — read each refuted claim against the answer before counting it.
...
### Runs 62 and 63 — 2026-09-19: Gemini 3.7 Flash (`medium`) and GPT-5.6 Luna (`high`), snapshot suite 8, format 8 board
- **Candidates**: Gemini 3.7 Flash @ `medium` (run 62); GPT-5.6 Luna @ `high` (run 63); parallel
  mode 2.
- **Prompt options**: mode 0, concise, tools, no web search, no subagents, source refs, spoiler-free
  off, snapshot yes, no history, no pre-injected wiki context.
- **Grading regime**: harness 36, scoring 12, Standard profile; Claude 5 Opus @ `medium`; Gemini 3.8
  Flash @ `medium` as second reader and claim verifier (the user's deliberate roster).
- **Instrument SHAs**: prompt `3ec0287f…`, ToolGuides `0863bd7b…`, KB `576ca574…`, wiki `473007b3…`,
  source `fcb8ac5b…`; `ChatService.cs` `de0b4e3`, `ToolGuides/` `7189f63`. Board 22,993 chars,
  `957f1f38c0a7`, format 8. **The first runs on this board; NotComparable with runs 54–61.**
- **Quality**: II 76 ± 3 and 73 ± 9 (hand counterfactual ≈ 77 and ≈ 74–75). Critical errors 0 and 1
  (Q11, genuine, third Luna run in a row).
- **Speed**: median model time 9,544 and 25,369 ms; TTFT P50 3,097 and 3,127 ms.
- **Cost**: US$3.88 and US$3.06; candidate US$0.38 and US$0.07; verifier US$1.85 and US$1.68 (48 %
  and 55 %; US$0.054 and US$0.044 per checked item). 42 and 64 tool calls, all succeeded; 1.1 and 1.7
  calls per round; cache read 81 % and 93 %.
- **Verification Outcome (runs 60 and 61 round)**: T1 line present where due but followed 1 time in 7,
  **rollback tripped, line removed**; W1 met; H1 notes not exercised; verifier cost missed on run 62
  and met on run 63; S1–S4 met; method-12 trigger not fired (1 and 0).
- **Verifier caution, instance 36**: 63 Q6, a potion's dice and cures refuted from the `case` block
  that only passes values computed at the top of the function (`src/potion.c:1784-1793` against
  `:966-1006`, `src/objects.c:3313-3317`).
- **Transfer Action**: harness 36 → **37** (`docs/overseer/ai-benchmark.md` § *Harness Version 37
  Updates*): short articles returned whole by `wiki_search` whatever scored (T4, rung 3); the *Without
  category* line and its guide sentence removed (T5); `get_artifact_stats` in Structured Lookup;
  denial-aware *Synthesis Divergence*; trigger labels; claim-verification instruction 3j; verifier
  spend and the raw text of a failed verification printed. Rung 2 wiki handoff W1 (Movement is
  non-somatic, four pages), W2 (how the Holy Grail is used), W3 (corrupt `Items/Blank paper.md`).
  Rubric repair S1–S6 (Q1, Q5, Q6, Q11, Q16, Q17; custom suite, no seed file). GnollHack plan G1–G5,
  not scheduled. No change to `ChatService.BuildSystemPrompt` or `_policy.md`; no `RecommendedModels`
  change.
- **Pre-declared criteria for the next run**: `benchmark_runs_62_63_analysis_v2.md` § 5.
- **Still outstanding from earlier rounds**: the runs 47–48 criteria and the runs 43–46 calibration
  re-assessment. Do not change `RecommendedModels` until both are done.

### Runs 64 and 65 — 2026-09-22: GPT-6 Luna (`high`) and GPT-5.6 Luna (`high`), snapshot suite 8, format 8 board
- **Candidates**: GPT-6 Luna @ `high` (run 64, first GPT-6 run); GPT-5.6 Luna @ `high` (run 65); parallel mode 2.
- **Prompt options**: mode 0, concise, tools, no web search, no subagents, source refs, spoiler-free off, snapshot yes, no history, no pre-injected wiki context; `clientSettings` null (unknown-input Controls branch).
- **Grading regime**: harness 37, scoring 12, Standard profile; Claude 5 Opus @ `medium`; Gemini 3.8 Flash @ `medium` second reader (blind) and claim verifier.
- **Instrument SHAs**: prompt `1b512e27…` (moved by `a0685ae`/`62ba369`: locations, controls, Elbereth), ToolGuides `cc4cf83a…`, KB `576ca574…`, wiki `3224a571…`, source `86d7a28d…`. Board 22,993 chars, `957f1f38c0a7`, format 8 — five formats behind GnollHack's 13.
- **Quality**: II 75 ± 8 and 75 ± 8. Critical errors 1 and 1, both Q11 (genuine; the unlearned book's castability, which format ≥ 9 prints).
- **Speed**: median model time 28,672 and 20,284 ms; TTFT P50 2,794 and 2,554 ms.
- **Cost**: US$2.20 and US$3.05; candidate US$0.043 and US$0.076; verifier 38 % and 50 % (US$0.041 and US$0.046 per checked item). 51 and 52 tool calls, all succeeded; cache read 93 % and 91 %.
- **Verification Outcome (runs 62 and 63 round)**: S1–S6 imported; T5 removal and T4 held; artifact stats counted; 3j held; spend lines printed; harness-36 definition-line note exercised for the first time (65 Q3) and correct; W1–W3 landed; verifier cost per item met on 64, missed by US$0.001 on 65.
- **Verifier caution, instances 37–39**: 64 Q11, 65 Q1, 65 Q8 — a Supported verdict covering a true clause beside the charged one; 2 of 4 validated evidence-informed withdrawals were wrong because of it.
- **Transfer Action**: harness 37 → 38 — the charged part judged (H1), accused sentences at Accuracy 5 (H2), claim totals in the synthesis (H3). Rung 3 T5 (`item_lookup` covers `Artifacts/`, moves `ToolGuidesSha256`) and T6 (`get_item_stats` minification keeps the failure reason and the invoked macro). Rung 2 W1–W4 wiki handoff. Rubric repair S1–S3 (Q6, Q7, Q10; custom suite, no seed file). Board refresh to format 13 requested (S4) for the next round. No change to `ChatService.BuildSystemPrompt` or `_policy.md`; no `RecommendedModels` change.
- **Pre-declared criteria for the next run**: `benchmark_runs_64_65_analysis_v2.md` § 5.
- **Still outstanding from earlier rounds**: the runs 47–48 criteria and the runs 43–46 calibration re-assessment.

### Runs 66 and 67 — 2026-09-24: GPT-6 Sol (`medium`) and GPT-5.6 Luna (`max`), snapshot suite 8, format 8 board
- **Candidates**: GPT-6 Sol @ `medium` (run 66, first Sol-6 run); GPT-5.6 Luna @ `max` (run 67); parallel mode 2; official endpoints.
- **Prompt options**: mode 0, concise, tools, no web search, no subagents, source refs, spoiler-free off, snapshot yes, no history, no pre-injected wiki context.
- **Grading regime**: harness 38, scoring 12, Standard profile; Claude 5.5 Opus @ `medium`; Gemini 3.8 Flash @ `medium` second reader (blind) and claim verifier.
- **Instrument SHAs**: prompt `1b512e27…`, ToolGuides `671d4949…`, KB `576ca574…`, wiki `3224a571…`, source `80b653e7…`. Board 22,993 chars, `957f1f38c0a7`, format 8. **The runs 64/65 rubric repair, wiki handoff and board refresh had not landed.**
- **Quality**: II 81 ± 4 and 76 ± 8. Critical errors 0 and 1 (Q11, genuine on the rubric; fifth Luna run with it, board-format lag).
- **Speed**: median model time 16,495 and 40,370 ms; TTFT P50 2,474 and 3,688 ms.
- **Cost**: US$2.72 and US$2.59; candidate US$0.69 and US$0.13; verifier 40 % and 56 % (US$0.05 and US$0.04 per checked item). 84 and 59 tool calls, all succeeded; cache read 90 % and 91 %.
- **Verification Outcome (runs 64/65 round)**: harness 38 landed; H2 (accused sentences at Accuracy 5) exercised on 67 Q1, Q2, Q7 and working; H3 claim totals in the synthesis present; H1 charged-part line did not prevent instance 42; T5 and T6 not exercised; S1–S3 not imported; W1–W4 not run; board refresh not done.
- **Verifier caution, instances 40–43**: a macro's definition line cited as evidence (66 Q13); a dispatcher read as the settling code (67 Q6); a Supported verdict on the uncharged clause (67 Q7); a constant's name read as the claim's meaning (67 Q15).
- **Transfer Action**: harness 38 → 39 (H1–H5), Task 3 dialog row; rung 3 T1, T2 (tool output; `ToolGuidesSha256` unchanged); rung 2 W5 (Holy Grail cures); rubric repair S1–S5 (custom suite, no seed file); GnollHack plan G1–G6. No change to `ChatService.BuildSystemPrompt`, `_policy.md` or `RecommendedModels`.
- **Still outstanding**: the runs 47–48 criteria, the runs 43–46 calibration re-assessment, the board refresh.
