...
this file's Git history before the runs 62 and 63 round.
...
### Runs 57 and 59 — 2026-09-19: Gemini 3.7 Flash (`medium`) and GPT-5.6 Luna (`high`), snapshot suite 8
- **Candidates**: Gemini 3.7 Flash @ `medium` (run 57); GPT-5.6 Luna @ `high` (run 59); parallel
  mode 2.
...
- **Grading regime**: harness 34, scoring 12, Standard profile; Claude 5 Opus @ `medium`; **Gemini
  3.8 Flash @ `medium` as second reader and claim verifier — a roster the run-56 runbook did not
  authorise** (it named GPT-5.6 Luna @ `high`).
- **Instrument SHAs**: prompt `4cc4361b…`, ToolGuides `1fb90bc0…`, KB `576ca574…`, wiki
  `58957b78…`, source `879630cb…`. Board 16,708 chars, `a21dc90405b6`, format 3.
- **Quality**: II 67 ± 11 and 67 ± 10 (hand counterfactual ≈ 71–72 and ≈ 69–70). Critical errors 3
  (Q7 spurious, Q11 overreach, Q3 genuine) and 2 (Q11 genuine, Q16 genuine falsehood, arguable cap).
- **Speed**: median model time 10,578 and 16,397 ms; TTFT P50 2,813 and 1,694 ms.
- **Cost**: $3.92 and $3.14; candidate $0.45 and $0.10; **verifier $2.06 and $1.66 (53 %)**. 51 and
  95 tool calls, all succeeded; 1.09 and 1.48 calls per round; cache read 77 % and 93 %.
- **Verification Outcome (run-56 round)**: C1 met (17,541 reasoning tokens printed); T1a and T1b met
  (Q16 148 k and 213 k input tokens); S1 met; H1 not exercised (no citation note printed); H3 partly
  missed (H1 of this round); H4 and the verifier-cost criterion read on a different verifier and
  therefore **not verified**; **method-12 trigger fired again** and is carried to the next round with
  the reason stated in the analysis § 4.
- **Verifier caution, instances 23–28**: 57 Q7 (function header cited), 57 Q11 (refuted and supported
  one fact in one run), 57 Q14 b (sequence read as a present-tense claim), 59 Q18 (misread its own
  cited table), 59 Q8 (substituted subject), 57 Q1 (a cited file that does not exist).
- **Transfer Action**: harness 34 → **35**: detector vocabulary, liveness note for a file not in the
  index, duplicate-claim removal, report and synthesis wording; rung 3 **T1** (`wiki_search.md`
  categories), **T2** (compiled-out note in the three source readers), **T3** (one `_policy.md`
  sentence); rubric repair S1–S5 (Q7, Q11, Q18, Q3, Q4; custom suite, no seed file); wiki handoff W1
  (`Skills/Martial arts.md`), under a validation template revised at the user's instruction so that
  only a factual, cited wording problem fails a validation and style remarks are optional
  suggestions (`server_wiki_handoff` § 3, § 4.2, § 4a); GnollHack plan G1–G6, deferred until the
  snapshot work finishes. Client-only: completion sound and notification made robust.
- **Pre-declared criteria for the next suite-8 run**: `benchmark_runs_57_59_analysis_v2.md` § 5.
