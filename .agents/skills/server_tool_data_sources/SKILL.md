...
**A dated fact to re-measure rather than to cite: what the dot-directory exclusion above removed.**
The GnollHack wiki's `WikiPath` is a git repository that also holds this project's own agent and
planning files, and before the run-30 exclusion the index took in **20 documents that are not
articles** — `.agents/skills/*/SKILL.md` (6), `.plans/**` (13) and `.vscode/` (1) — so five
documents titled `skill`, four `walkthrough`, four `implementation_plan_v1` and three
`implementation_plan_v2` were searchable as GnollHack wiki articles, indistinguishable from genuine
ones by `wiki_search` or `wiki_view`. The same pass measured **32 colliding titles across 72 files,
26 of them genuine article pairs** — the fact that motivated `wiki_view`'s disambiguation payload
(`server_tool_parameter_reference` § 5). Both figures are a measurement of one revision,
`ade24a4` (`WikiHeadSha`), and will drift as the wiki repository changes; re-measure before citing
either again rather than repeating this paragraph as current.
