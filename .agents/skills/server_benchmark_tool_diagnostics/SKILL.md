...
>
> **A recorder cut is not a `ToolExecutor` cut.** `BenchmarkToolCallRecorder` applies its own storage cap, `Benchmark:ToolCallRecord:MaxResultChars`, after `ToolExecutor` has handed the result over. Up to harness 25 that cap was `Benchmark:MaxResultLength` + 2,000 = 12,000 — below `wiki_search`'s 13,000 and `nethack_wiki_search`'s 16,140 overrides — and it marked its cut with the same `... [Result truncated for length]` string as the pre-run-28 `ToolExecutor`: run 41 Q11's first `wiki_search` call returned 12,221 characters, stored 12,000, and its export tail reads exactly like a pre-run-28 cut. The tell is the row — `ResultTruncated = yes` beside a stored length below `ResultLengthChars`. **Never date a run by that string when the column says the recorder cut it.** From harness 26 the recorder's cap is the larger of `Benchmark:MaxResultLength` and the largest `MaxResultLengthOverride` among the allowed tools, plus 2,000, and its marker names itself: `... [Record truncated: stored N of M characters]`.
>
...
>
> **From harness 31 a filtered `source_code_search` miss carries a second payload marker.** When the call had a `file_filter` and a literal query, the miss adds, directly after its first sentence, one of: *"Without file_filter, '<query>' matches N lines across a.c, b.c."* (with *"(top 3 files shown)"* when three are named — never a corpus total), *"The same search without file_filter found no match either."*, or *"The unfiltered check could not be run."* The first means the **filter** excluded the match and the query was right; the second means one literal, line-bounded, case-insensitive substring test found nothing unfiltered either — not that the text is absent from the corpus, since files over `MaxSourceFileSizeKB` are not indexed at all. The miss is still a miss for counting; the marker says why.

> **From harness 31 the report counts misses for you — start there.** The Tool Usage Profile prints *"Not-found results: N of K inspectable successful payloads (tool ×n, …); U payloads unavailable"* and *"Cut before the model saw it: N by the per-tool cap; batch budget and turn limit not recorded. Cut in the stored record only: M"*, and the per-question table and the tool-call log carry a `Note` column (`miss`, `cut`, `record cut`, `partial`, `content error`, `unavailable`). They come from `BenchmarkToolResultClassifier`, which matches each tool's miss opening at the start of the result (never a length), treats a payload the retention sweep nulled as **unavailable — neither a hit nor a miss**, and reports a failed `get_knowledge_article` not-found apart, because it is the one miss returned as a failure. Use those lines as the **first stop** for a miss or cut count, then spot-check rows against the opening table in `server_tool_parameter_reference`. Two limits: the batch budget and the turn limit cut a result *after* `AgentLoopRunner` stores it, so a cut by either is invisible in the rows; and the lines are computed from stored rows, so a report re-rendered from harness-31 code prints them for any run from harness 17 on, while a run without rows gets none and is counted by hand as above.
...
**A `[Not compiled: …]` line is a harness-35 marker (the runs 57 and 59 round, 2026-09-19).** From
the round `get_function_definition` and `source_code_view` end a result whose lines fall inside a
region opened by a literal `#if 0` with one line, at most **300 characters**, naming those file line
ranges and saying they are removed or disabled code; `source_code_search` adds
`[Not compiled: the match at <file>:<line> is inside #if 0 ... #endif.]` for up to two `>>>`-marked
matches in such a region. The line is tool output on an ordinary success, for `.c` and `.h` files of
either repository, written by `Overseer/Services/Tools/SourceCompiledOutNote.cs`; no other
preprocessor condition is treated as compiled out (`server_tool_parameter_reference` § 4 states the
contract). **Its absence dates a run**: a result showing `#if 0` lines with no such note is a run
recorded before this round — run 59 Q16's `study_book` body (`src/spell.c:933-960`), read as live,
is the case that motivated it. In a later run, a result that shows `#if 0` lines without the note is a
regression of the contract, and a note on code that is compiled is a false positive worth a finding.

**A `[Without category, the best match for this query is …]` line is a harness-36-only marker (added
in the runs 60 and 61 round, removed in the runs 62 and 63 round, both 2026-09-19).** Under harness 36
a `wiki_search` call that set `category` and returned results carried one line naming the single best
match for the same query **without** the category, when that article's path did not contain the
category and the article was not among the results: `[Without category, the best match for this
query is <path>, which is outside '<category>'. Omit category to see it.]`, at most 240 characters,
after the snippets and ahead of the *Showing N of M* line — or first, when the result was within 400
characters of its 13,000-character cap. **Its presence dates a run to harness 36 exactly**: no run
stamped 35 or earlier, or 37 or later, carries it. From harness 37 a categorised non-empty result is
the snippets and the *Showing N of M* line only (`server_tool_parameter_reference` § 5); a line in a
run stamped 37 or later means the removal did not deploy. The empty-result miss payload names the
unfiltered match under every one of these harnesses. Run 63 carried the line on 10 of 17 categorised
searches and the model followed it once in seven chances, which tripped the rollback.
...
> This pass reads stored run columns only. **For any run before harness 17, arguments and results were never stored** for any benchmark tool call (`ShowDebugLog` is `false` at every benchmark call site, and a run creates no `ChatMessage` rows), so every statement about such a call's parameters or its returned content is a reconstruction or a replay, labelled as such. **From harness 17, a run's `BenchmarkRunAnswerToolCall` rows carry the real arguments, result, error, status, emission order and timings for every attempted call** — read them through the tool-calls endpoint (§ 7, rung 0) rather than reconstructing, unless the payload columns were later pruned by the retention sweep (`ChatRetentionSettings.PruneBenchmarkToolCallResultsDays`, default 90 days), in which case `ArgsText`/`Result` are null but `Name`, `Status`, `Error` and `ResultLengthChars` still are not. **Two corpora reachable from this run carry no fingerprint** — the NetHack source (`NetHackSourceCodePath`) and the NetHack wiki (`NetHackWikiPath`) — so any NetHack finding rests on `StartedAtUtc` against the corpus as it stands now. **Any run before harness 16 has no GnollHack wiki or GnollHack source provenance at all**: `WikiHeadSha` and `SourceCodeHeadSha` were added in harness 16, no historical row is backfilled and none can be, and null in either column means *not recorded*. **From harness 32 the report prints every accused sentence the harness sent to the claim verifier with its verdict** (per answer, *"Accused sentences checked: N — supported a, refuted b, indeterminate c"*, and the run totals under Assessor Findings), computed at render time from `ClaimVerificationJson`, so a report re-rendered from harness-32 code shows them for any run that stored roles; before that, an accused sentence's outcome was visible only when Supported. **From harness 33 a verification item's role says whose sentence it checked**: `unverifiedClaim` and the accused-sentence roles are the candidate's, while `assessorStatement` is a sentence of the assessor's own evidence, whose Refuted verdict means the candidate was right. Only the candidate's items count as refuted claims. Before harness 33 assessor sentences were stored as `unverifiedClaim` and cannot be told apart by role. **Also from harness 33, a verdict that cites a `src/*.c` line inside a function with no live call site in the indexed GnollHack source carries `citationNote`** (*"cited function <name> has no live call site"*) and counts as Indeterminate; read the stored verdict and the note together, and remember the check cannot see calls made through a function pointer or a macro. **From harness 34 a citation note needs a definition followed by a body and no reference of any kind**: a column-0 line counts as the enclosing function only when its parameter list is followed by `{` (a macro row in a data table such as `SCROLL("mail", …),` is no function), and a value reference — the name passed or stored as a function pointer — counts as live alongside a `name(` call, so a function reached through a function pointer no longer gets a note. A note on a harness-33 run may be one of those two blind spots; check the cited line before relying on it. From harness 34 a `source_code_search` or `source_code_view` result may end with a `[Definition: …]` pointer line; it is part of an ordinary success (`server_tool_parameter_reference` § 4). **From harness 35 a verdict that cites a `src/<file>` not in the indexed GnollHack source carries the note** *"cited file <path> is not in the indexed source"* and counts as Indeterminate, as the liveness note does; a file over `MaxSourceFileSizeKB` is also "not in the index", so read the note as "not indexed", never as "does not exist". From harness 35 a `get_function_definition`, `source_code_view` or `source_code_search` result may carry a `[Not compiled: …]` line naming lines inside `#if 0`; it too is part of an ordinary success (§ 4 above). **From harness 36 two more notes demote a verdict to Indeterminate**: *"cited file <path> without a line"*, when the only source citation names a `src/` or `include/` file with no line and nothing else in the citation names a wiki page or a board line, and *"cited line <path>:<line> is only the definition line of <function>"*, when the only source evidence is a single line that is a function's definition line (a range is never noted this way). Like the others, they are written when the verdict is stored and never recomputed, so a run stamped 35 or earlier carries neither, and a right verdict with such a citation loses its count by design. A run stamped 36, and only 36, may carry a `[Without category, the best match for this query is …]` line on a categorised `wiki_search` result; it was part of an ordinary success (§ 4 above). **From harness 37 the report prints the verifier's spend per answer** — each *Claim Verification* line ends with its tool calls, input tokens and seconds, and *Harness Cost* names the answers the verifier spent most on with the per-answer means — and **a failed verification's line carries the first 600 characters of `ClaimVerificationRawText`**. Both are computed at render time from stored columns, so a report of an earlier run re-rendered from harness-37 code shows them wherever the columns were stored; the verifier's own tool calls are still not recorded one by one. **From harness 38 an accused item's verifier prompt carries its charged part** — a *Charged part (the words the assessor quoted, …)* line whenever the quoted fragments are shorter than the sentence — and the verifier is told to judge that part, not the whole sentence; **accused sentences are checked at Accuracy 5 as well** as at 4 and below. A Supported verdict on an accused sentence or a critical-error quote in a run stamped 37 or earlier may cover a different clause than the charged one (runs 64 Q11, 65 Q1, 65 Q8), so read its basis against the assessor's quoted words before counting it. **From harness 39 a verdict whose only source evidence is a single line inside a `#define` header** — the `#define` line or one of its backslash-continued lines — carries *"cited line <path>:<line> is only the definition of macro <NAME>"* and counts as Indeterminate, as the harness-36 definition-line note does; a range is never noted. **Also from harness 39 an accused item with a charged part is counted by its own `chargedPartVerdict`**, judged on the charged part alone with its own citation in `chargedPartBasis`; the item's `verdict` is kept for display but no longer counts, a missing or unparseable `chargedPartVerdict` counts as Indeterminate with the note *"the charged part was not judged separately"*, and the citation notes apply to `chargedPartBasis` for such an item. A run stamped 38 or earlier carries no charged-part verdicts, and its stored verdicts are unchanged. Two tool-output changes shipped in the same round, and neither moved `ToolGuidesSha256`: a `wiki_view` result without a `section` whose rendered article is longer than the per-tool cap **begins** with an `[Article is N characters; the first M are shown. Headings: …. Call wiki_view again with section set to one of them to read the rest.]` line and carries no `[Truncated:` suffix, so the tool-call log marks it **`partial`**, not `cut`, and it counts under partial results rather than the per-tool cap, and the `item_lookup` / `monster_lookup` result header (and live chat's pre-injected wiki context) names the repository path, `--- Items/Torch.md ---`, rather than the bare filename. Both are parts of an ordinary success (`server_tool_parameter_reference` § 5). `BenchmarkAssessmentPrompt.HarnessVersion` is now `"39"`.
