...
**A `[Without category, the best match for this query is …]` line is a harness-36 marker (the runs 60
and 61 round, 2026-09-19).** From the round a `wiki_search` call that sets `category` and returns
results carries one line naming the single best match for the same query **without** the category,
when that article's path does not contain the category and the article is not among the results:
`[Without category, the best match for this query is <path>, which is outside '<category>'. Omit
category to see it.]`, at most 240 characters, after the snippets and ahead of the *Showing N of M*
line — or first, when the result is within 400 characters of its 13,000-character cap. It is tool
output on an ordinary success (`server_tool_parameter_reference` § 5 states the contract); the
empty-result miss payload already named the unfiltered match and is unchanged. **A categorised
`wiki_search` result in a run stamped 36 or later whose best unfiltered match lies outside the category
and that lacks the line is a regression; its absence dates an earlier run** — run 61's five
categorised searches that hid the root-level mechanics article (Q3, Q11 ×2, Q16 ×2) are the cases that
motivated it. A line on a result that already contains the best unfiltered match is a false positive
worth a finding. The line fires on the *best* match, which can be a weak one; it is a pointer, not
evidence that the category was wrong.
