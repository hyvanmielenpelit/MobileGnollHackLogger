...
public class BenchmarkArtifactScrubberTests
{
    ...
    // ---------------------------------------------------------------------------------------
    // Shielded narration — 2026-09-03 Luna run, Q3 and Q5.
    //
    // Both answers were flagged ReasoningBleed and the report asserted that narration had been
    // removed before grading, yet both reached the assessor with the narration intact and were
    // docked for it: Q3 scored 55/55 on conciseness and readability against 87/87 on comparable
    // answers, and the assessor named "transport/preamble filler" and "leftover internal search
    // preamble" as the reason. The narration strip stopped at the first paragraph it did not
    // recognise, so one unrecognised opener hid every narration paragraph behind it.
    // ---------------------------------------------------------------------------------------

    private const string Q3Answer =
        "Silver dragon scale mail has these GnollHack stats:\n\n" +
        "- **Armor type:** Suit of armor\n" +
        "- **Magic cancellation:** **+4**\n" +
        "- **Weight:** **34.4 lb** (550 weight units)";

    [Fact]
    public void Q3_OrphanTokenShieldingNarration_BothRemoved()
    {
        // The blank lines between the token, the narration and the answer were padded with
        // single spaces in the run; WhitespaceOnlyLineRegex normalises them before splitting.
        string raw =
            "tsotlhe \n\n \n\n" +
            " I’m checking the armor reference for how GnollHack presents that penalty code.\n\n" +
            " \n\n" +
            Q3Answer;

        var result = Scrubber.Scrub(raw);

        Assert.Equal(Q3Answer, result.AnswerText);
        Assert.DoesNotContain("tsotlhe", result.AnswerText);
        Assert.DoesNotContain("I’m checking", result.AnswerText);
        Assert.Equal(1, result.NarrationBlockCount);
        Assert.True(result.Flags.HasFlag(BenchmarkAnswerFlags.ReasoningBleed));
        Assert.Contains("tsotlhe", result.ArtifactText);
    }

    private const string Q5Answer =
        "GnollHack uses a **turn-based prayer countdown**, not a fixed “wait 350 turns” rule.\n\n" +
        "- The counter decreases by **1 per game turn**.\n" +
        "- With **major trouble**, a counter of **200 or less** is acceptable.";

    [Fact]
    public void Q5_UnrecognisedNarrationOpener_NoLongerShieldsTheParagraphsBehindIt()
    {
        string raw =
            "I found the relevant implementation; the remaining detail is the randomized " +
            "duration, which is generated by GnollHack’s `rnz()` routine rather than a fixed " +
            "interval.\n\n" +
            "I’m verifying the `rnz()` distribution so I can distinguish the base timer from " +
            "its expected and extreme values. \n\n" +
            "I’m reading the random-number helper once more to avoid overstating the possible " +
            "spread.\n\n" +
            Q5Answer;

        var result = Scrubber.Scrub(raw);

        Assert.Equal(Q5Answer, result.AnswerText);
        Assert.DoesNotContain("I found the relevant implementation", result.AnswerText);
        Assert.DoesNotContain("I’m verifying", result.AnswerText);
        Assert.DoesNotContain("I’m reading", result.AnswerText);
        Assert.Equal(3, result.NarrationBlockCount);
        Assert.True(result.Flags.HasFlag(BenchmarkAnswerFlags.ReasoningBleed));
    }

    // ---------------------------------------------------------------------------------------
    // Negative fixtures for the shield rules. Reaching past an unrecognised paragraph is
    // exactly the reach that could take an answer's own opening, so what must NOT move is
    // pinned harder than what must.
    // ---------------------------------------------------------------------------------------

    [Fact]
    public void ShortAuthoredOpeningLine_IsNotSteppedOverAndLost()
    {
        string raw =
            "There are exactly three.\n\n" +
            "Let me look at each in turn — Elbereth, Gilthoniel and Morgoth are the only " +
            "runewords GnollHack implements, and each works in a different branch of the " +
            "dungeon with its own restrictions on who ignores it.";

        var result = Scrubber.Scrub(raw);

        // "Let me look" matches the narration signature, but it is this answer's own prose.
        // The short opening ahead of it must survive, and so must the paragraph itself.
        Assert.StartsWith("There are exactly three.", result.AnswerText);
        Assert.Contains("Elbereth, Gilthoniel and Morgoth", result.AnswerText);
    }

    [Fact]
    public void OneWordAnswerLineWithPunctuationOrMarkup_IsNotTreatedAsAnOrphanToken()
    {
        string raw =
            "**Yes.**\n\n" +
            "A blessed scroll of identify identifies three items, and the count does not " +
            "depend on how many unidentified items you are carrying.";

        var result = Scrubber.Scrub(raw);

        Assert.StartsWith("**Yes.**", result.AnswerText);
    }

    [Fact]
    public void AnswerThatIsEntirelyNarration_IsLeftIntactRatherThanEmptied()
    {
        string raw =
            "I’m checking the armor reference.\n\n" +
            "I’m verifying the distribution.";

        var result = Scrubber.Scrub(raw);

        // Better to hand the assessor narration than to hand it nothing: an empty answer is
        // scored as a transport defect and excluded, which is a far worse misreading.
        Assert.False(string.IsNullOrWhiteSpace(result.AnswerText));
    }

    [Fact]
    public void MarkdownOpeningAheadOfNarration_IsNotSteppedOver()
    {
        string raw =
            "### Prayer timeout\n\n" +
            "I’m verifying the `rnz()` distribution before answering.\n\n" +
            "The counter decreases by one per game turn, and praying is safe once it reaches " +
            "zero, or 200 with major trouble.";
